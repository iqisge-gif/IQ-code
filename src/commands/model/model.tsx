...
export const call: LocalJSXCommandCall = async (onDone, _context, args) => {
  ...
  if (COMMON_HELP_ARGS.includes(args)) {
    onDone(`Run /model to open the model selection menu, or /model [modelName] to set the model.${getCurrentProviderHint()}`, {
      display: 'system'
    });
    return;
  }
...
function getCurrentProviderHint(): string {
  const { provider, index } = getCurrentCustomApiProviderWithIndex();
  return provider?.name ? ` Current provider: ${provider.name} (#${String(index + 1).padStart(3, '0')}).` : '';
}
