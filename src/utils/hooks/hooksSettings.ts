...
export function getAllHooks(appState: AppState): IndividualHookConfig[] {
  ...
  if (!restrictedToManagedOnly) {
    ...
    // both resolve to ~/.doge/settings.json)
...
export function hookSourceDescriptionDisplayString(source: HookSource): string {
  switch (source) {
    case 'userSettings':
      return 'User settings (~/.doge/settings.json)'
    case 'projectSettings':
      return 'Project settings (.doge/settings.json)'
    case 'localSettings':
      return 'Local settings (.doge/settings.local.json)'
    case 'pluginHook':
      ...
      // e.g., "Plugin hooks (~/.doge/plugins/repos/source/example-plugin/example-plugin/hooks/hooks.json)"
      return 'Plugin hooks (~/.doge/plugins/*/hooks/hooks.json)'
    case 'sessionHook':
