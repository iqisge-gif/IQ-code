 ...
 *
 * The enabled/disabled state remains in .doge/settings.json for per-repo control.
 *
 ...
 *
 * Legacy cache structure: ~/.doge/plugins/cache/{plugin-name}/
 * Versioned cache structure: ~/.doge/plugins/cache/{marketplace}/{plugin}/{version}/
 *
...
function migrateV1ToV2(v1Data: InstalledPluginsFileV1): InstalledPluginsFileV2 {
  ...
  for (const [pluginId, plugin] of Object.entries(v1Data.plugins)) {
    // V2 format uses versioned cache path: ~/.doge/plugins/cache/{marketplace}/{plugin}/{version}
